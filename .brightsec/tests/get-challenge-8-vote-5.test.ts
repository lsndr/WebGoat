...
test('GET challenge/8/vote/5', { signal: AbortSignal.timeout(timeout) }, async () => {
  await runner
    .createScan({
    ...
    .run({
      ...
      url: `${baseUrl}/challenge/8/vote/5`,
      headers: {
        'X-FlagController': 'Thanks for voting, your flag is: <flag_value>'
      }
    });
});